...
local src = assert(slurp("gen2pic.lua"))
...
-- The plate arm that used to sit on top of this lived in the Gen1Wild bundle's
-- theme runtime, not in this mod, so the assertions about its removal stay
-- with the bundle's suite.  What belongs here is above: where the pic comes
-- from, and what this mod hands the cart when it has none.
...
local Pic = assert(loadfile("gen2pic.lua"))()(mod, {})
...
do -- a whole sheet missing does not become 251 log lines
  ...
  warnings = {}
  local P = assert(loadfile("gen2pic.lua"))()(mod, {})
...
local mainSrc = assert(slurp("main.lua"))
