...
local src = assert(slurp("gen2unseen.lua"))
local gen2Src = assert(slurp("gen2.lua"))
...
local picSrc = assert(slurp("gen2pic.lua"))
...
local Unseen = assert(loadfile("gen2unseen.lua"))()(makeMod(), {})
...
do -- the row is live and gates the press
  local Off = assert(loadfile("gen2unseen.lua"))()(
...
do -- the mask failing must not become the leak it exists to prevent
  ...
  local Boom = assert(loadfile("gen2unseen.lua"))()(makeMod(), {})
